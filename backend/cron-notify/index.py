...
def send_payment_reminders(cur, conn, check_date, settings):
    ...
    for days in reminder_days:
        ...
        if days == 0:
            reminders.append(('reminder_today', target_date, 'pending', tpl_payment_today_title, tpl_payment_today_body))
        elif days == 1:
...
def send_savings_reminders(cur, conn, check_date, settings):
...
def send_telegram_payment_reminders(cur, conn, check_date, settings):
    ...
    for days in reminder_days:
        target_date = (today_date + timedelta(days=days)).isoformat()
        if days == 0:
            reminders.append(('tg_reminder_today', target_date, 'pending', tpl_payment_today))
        elif days == 1:
...
def send_telegram_savings_reminders(cur, conn, check_date, settings):
...
def send_max_payment_reminders(cur, conn, check_date, settings):
    if settings.get('enabled', 'false') != 'true':
    ...
    for days in reminder_days:
        target_date = (today_date + timedelta(days=days)).isoformat()
        if days == 0:
            reminders.append(('max_reminder_today', target_date, 'pending', tpl_payment_today))
        elif days == 1:
...
def send_max_savings_reminders(cur, conn, check_date, settings):
    if settings.get('savings_enabled', 'false') != 'true':
