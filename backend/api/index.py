    ...
    for pay in payments:
        ...
        if is_manual:
            ...
            for row in unpaid_rows:
                ...
                # Будущий период (другого календарного месяца), который платёж не может
                # покрыть ПОЛНОСТЬЮ: по ст. 319 ГК РФ деньги должны в первую очередь
                # гасить начисленные проценты (и пеню), а не сразу основной долг.
                # Засчитываем частично — проценты, затем пеня, ОД не трогаем — и на этом
                # периоде останавливаемся. ВАЖНО: период ТЕКУЩЕГО месяца (is_current_month)
                # сюда не подпадает — это не «будущий» в смысле ст.319, а обычный текущий
                # период, который должен гасить ОД как всегда, иначе излишек «зависает»
                # без привязки к периоду и период остаётся просроченным, хотя деньги внесены.
                if is_future and not is_current_month and remaining < need_total - Decimal('0.005'):
        ...
        if action == 'create':
            ...
            if is_full_early_repayment:
                ...
                for row in unpaid_rows:
                    ...
                    # Будущий период (другого календарного месяца), который платёж не может
                    # покрыть ПОЛНОСТЬЮ: по ст. 319 ГК РФ деньги должны в первую очередь
                    # гасить начисленные проценты (и пеню), а не «прыгать» сразу в основной
                    # долг. Засчитываем частично — сначала проценты, затем пеня, ОД периода
                    # не трогаем — и на этом периоде останавливаемся. Период ТЕКУЩЕГО месяца
                    # (is_current_month) сюда не подпадает — иначе излишек «зависает» без
                    # привязки к периоду, и период остаётся просроченным, хотя деньги внесены.
                    if is_future and not is_current_month and remaining_amt < need_total - Decimal('0.005'):
...
def handle_savings(method, params, body, cur, conn, staff=None, ip=''):
    if method == 'GET':
        ...
        if action == 'create':
            ...
            """Выплата остатка по уже закрытому/досрочно закрытому договору — обнуление баланса и процентов"""
            sid = int(body['saving_id'])
            payout_date = body.get('payout_date', date.today().isoformat())
            note = body.get('note', '')
            cur.execute("SELECT id, contract_no, current_balance, accrued_interest, status FROM savings WHERE id=%s" % sid)
            sv = cur.fetchone()
            if not sv:
                return {'error': 'Договор не найден'}
            sid, contract_no, balance, accrued, status = sv
            balance = Decimal(str(balance))
            accrued = Decimal(str(accrued or 0))
            ...
            cur.execute(
                "INSERT INTO savings_transactions (saving_id, transaction_date, amount, transaction_type, description) VALUES (%s, '%s', %s, 'final_payout', '%s')" % (
                    sid, payout_date, float(total), desc.replace("'", "''")
                )
            )
            cur.execute("UPDATE savings SET current_balance=0, accrued_interest=0, updated_at=NOW() WHERE id=%s" % sid)
...
def generate_pd_consent_docx(fio, passport_series, passport_number, passport_issued_by, passport_issue_date, passport_dept_code):
    ...
    intro_text = (
        ...
        'Организация «ЭКСПЕРТ ФИНАНС» ИНН 4307012081 (Адрес местонахождения 346500, Ростовская область, г Шахты, пр-кт Пушкина, зд. 29а ), '
        'Организация «ФИН ФОРМУЛА» ИНН 3666209530 (Адрес местонахождения 346500, Ростовская область, г Шахты, пр-кт Пушкина, зд. 29а, помещение 1), '
